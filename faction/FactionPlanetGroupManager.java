...
// TODO: look for static Map references; we have an instance to use for more accuracy
// TODO: make a completely different planet group manager for ivits 
public class FactionPlanetGroupManager implements IFactionPlanetGroupManager {
	 ...
	 */
	public FactionPlanetGroupManager(Map map) {
		this.map = map;
	...
	private int connectGroups(Vector<PlanetGroup> minimumfeds, Vector<PlanetGroup> chosengroups, Vector<Coordinates> possiblesatellites,
			Vector<PlanetGroup> connectedgroups, int currentpower, Vector<Coordinates> placedsatellites, Vector<Coordinates> chain, int leastsatellites, boolean valid) {
		
		// don't try to add more satellites if more would be used than the current minimum
		// each new satellite can only add two new planet groups, so we're at least that many satellites away from being done
		// TODO: potentially trade memory for CPU here and not recalculate this every time
		int missinggroups = 0;
		for (PlanetGroup g : chosengroups) if (!connectedgroups.contains(g)) missinggroups++;
		if (placedsatellites.size() + (missinggroups + 1) / 2 >  leastsatellites) return leastsatellites;
		

		Vector<Coordinates> candidates = null;
		if (chain != null) {
			// if a new satellite chain that hasn't reached the next planet group has been started, extend it
			candidates = new Vector<Coordinates>();
			Coordinates end = chain.get(chain.size() - 1);
			Vector<Coordinates> neighbors = map.neighbors(end);
			neighbors:
			for (Coordinates c : neighbors) if (possiblesatellites.contains(c)) {
				// the new satellite chain can't touch any already connected planet group or any other existing satellite except the end of the chain
				// we have or will already check that path starting from the other end, or the chain is more efficient without the beginning links
				for (PlanetGroup g : connectedgroups) if (g.adjacent(c)) continue neighbors;
				for (Coordinates p : placedsatellites) if ((p != end) && map.adjacent(c, p)) continue neighbors;
				candidates.add(c);
			}
		} else {
			// if a new satellite chain hasn't started, start a new one anywhere from any available adjacent position
			chain = new Vector<Coordinates>();
			candidates = possiblesatellites;
		}
		
		// score satellites in terms of most likely improvement
		for (Coordinates s : candidates) {
			int score = 0;
		...
		for (Coordinates s : candidates) {
			...
			for (PlanetGroup g : chosengroups) g1.merge(g);
			...
			*/
			...
			if (complete) {
				...
				if (valid && rule3Valid(connectedgroups, newpower, placedsatellites))
					g.federate();
				if (!minimumfeds.contains(g)) minimumfeds.add(g);
			} else {
				...
				// establish the chain object for recursion, which is null if new groups have been joined, or extended with the new satellite otherwise
				Vector<Coordinates> newchain = null;
				if (groupsadded == 0) {
					chain.add(s);
					newchain = chain;
				}
				
				// create a new vector of all possible new satellites, at the very least adding all the just-placed satellite's neighbors and removing itself;
				// we must also add empty cells next to newly-added planet groups
				Vector<Coordinates> newcandidates = (Vector<Coordinates>)possiblesatellites.clone();
				newcandidates.remove(s);
				for (Coordinates neighbor : map.neighbors(s)) {
					if (!newcandidates.contains(neighbor) && !placedsatellites.contains(neighbor) && emptyspaces.contains(neighbor)) newcandidates.add(neighbor);
				}
				int connectedgroupcount = connectedgroups.size();
				for (int i=0; i < groupsadded; i++) {
					PlanetGroup g = connectedgroups.get(connectedgroupcount - i - 1);
					Vector<Coordinates> neighbors = g.neighbors();
					for (Coordinates n : neighbors) if (!newcandidates.contains(n) && !placedsatellites.contains(n) && emptyspaces.contains(n))
						newcandidates.add(n);
				}
				// recurse and add more satellites until all groups are added
				int least = connectGroups(minimumfeds, chosengroups, newcandidates, connectedgroups, newpower, placedsatellites, newchain, leastsatellites, isvalid);
				if (least < leastsatellites) leastsatellites = least;
			}
			for (int i=0; i < groupsadded; i++) connectedgroups.remove(connectedgroups.size() - 1);
			if (groupsadded == 0) chain.remove(chain.size() - 1);
	...
	private void addPossibleFederations(Vector<PlanetGroup> groups) {
		...
		*/
		...
		connectGroups(minimumfeds, groups, possiblesatellites, connectedgroups, g.buildingvalue(), placedsatellites, null, MAXIMUMSATELLITES - usedsatellites, true);
